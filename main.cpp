...
void connect_triangles(SDL_Renderer* const renderer, const std::vector<Point> &points, int i, int j, int k) {

    SDL_Vertex vertices[3];
    int indexes[3] = {i,j,k};

    for(int cur = 0; cur < 3; cur++ ) {
        SDL_FPoint point = {(float) points[indexes[cur]].x, (float) points[indexes[cur]].y};
        SDL_Vertex vertex = {{point}, {255, 0, 0, 255}, {1, 1}};
...
int main(int argc, char** argv) {
    ...
    while (!close) {
        ...
        connect_triangles(renderer, points, 0,1,2);
